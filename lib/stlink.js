...
export default {
  ...
  notifySuccess(message) {
    ...
    if(this.stlinkModalPanel.isVisible()) {
      ...
      if(body) {
          m = m + '<br/><div style="max-height: 100px; max-width: 520px; overflow: auto"><pre>' + body + '<pre></div>'
      }
      this.StlinkView.setOutput(m);
      this.StlinkView.showOutput(true);
    }
    else {
      // body is just too much blah blah for notification ...
      /*
      if(body) {
      ...
      */
      atom.notifications.addSuccess('stlink flash-tool', {detail: 'device programming was successful'});
    }
  },
  ...
  toggle() {
    ...
    if(prog.killRunning()) {
      if(this.stlinkModalPanel.isVisible()) {
          ...
          this.stlinkModalPanel.show();
          this.StlinkView.flashButton.focus();
      }
    }
  }
};
